...
Сбросим темы GTK у пользователя к дефолтным (могут вызвать чёрный экран при входе в сессию):
...
В клиенте ThinLinc при входе поставьте галочку **«Пересоздать сессию»**, чтобы переменные окружения применились.
